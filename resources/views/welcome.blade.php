...
<html lang="en">
  ...
  <ul class="dropdown-menu dropdown-menu-end shadow p-2" style="width: 300px; max-height: 400px; overflow-y: auto;">
    ...
    @forelse ($userResearches as $research)
    <li>
      <a href="#" class="dropdown-item py-2 d-flex flex-column">
        <span class="fw-semibold text-dark">
          📘 Research titled "{{ $research->title }}" has been assigned to you.
        </span>
        <small class="text-muted">{{ $research->created_at->diffForHumans() }}</small>
      </a>
    </li>
    ...
    @forelse ($userExtensions as $extension)
    <li>
      <a href="#" class="dropdown-item py-2 d-flex flex-column">
        <span class="fw-semibold text-dark">
          🧩 Extension titled "{{ $extension->title }}" has been assigned to you.
        </span>
        <small class="text-muted">{{ $extension->created_at->diffForHumans() }}</small>
      </a>
    </li>
    <li><hr class="dropdown-divider my-1"></li>
    @empty
    ...
    @if ($userResearches->isEmpty() && $userExtensions->isEmpty())
            ...
            <table class="table table-hover align-middle w-100" id="researchTable">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="fw-semibold">{{ $research->title }}</td>
                  <td>
                    <div class="text-truncate" style="max-width: 400px;" title="{{ $research->description }}">
                      {{ $research->description }}
                    </div>
                    <small class="text-muted d-block mt-1" style="font-size: 0.85em;">
                    {{ \Carbon\Carbon::parse($research->start_date)->format('M d, Y') }} - 
                    {{ \Carbon\Carbon::parse($research->end_date)->format('M d, Y') }}
                    </small>
                  </td>
                  <td>
                    <span class="{{ $statusClass }}" style="font-size: 0.9em; min-width: 80px; display: inline-block; text-align: center; padding: 0.25em 0.6em; border-radius: 0.375rem;">
                    {{ ucfirst($research->status) }}
                    </span>
                  </td>
                  <td>
                    <a href="#" class="btn btn-outline-primary btn-sm rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#viewModal{{ $research->id }}">
                    View
                    </a>
                    <!-- Modal -->
                    <style>
          ...
          <div class="table-responsive">
            <table class="table table-hover align-middle" id="extensionTable">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{ $loop->iteration }}</td>
                  <td class="fw-semibold">{{ $extension->title }}</td>
                  <td>
                    <div class="text-truncate" style="max-width: 400px;" title="{{ $extension->description }}">
                      {{ $extension->description }}
                    </div>
                    <small class="text-muted d-block mt-1" style="font-size: 0.85em;">
                    {{ \Carbon\Carbon::parse($extension->start_date)->format('M d, Y') }} - {{ \Carbon\Carbon::parse($extension->end_date)->format('M d, Y') }}
                    </small>
                  </td>
                  <td>
                    <span class="{{ $statusClass }}" style="font-size: 0.9em; min-width: 80px; display: inline-block; text-align: center; padding: 0.25em 0.6em; border-radius: 0.375rem;">
                  ...
                  <td>
                    <a href="#" class="btn btn-outline-primary btn-sm rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#viewExtensionModal{{ $extension->id }}">View</a>
                    <!-- Modal -->
                    <div class="modal fade" id="viewExtensionModal{{ $extension->id }}" tabindex="-1" aria-labelledby="extensionModalLabel{{ $extension->id }}" aria-hidden="true">
                      <div class="modal-dialog modal-xl modal-xl-custom modal-dialog-centered">
                        <div class="modal-content">
              <div class="modal-header">
